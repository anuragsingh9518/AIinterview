...
export const Navbar = () => {
    ...
    const [ showCreaditPopup,setShowCreditPop]=useState(false)
    const [ showUserPopup,setShowUserPop]=useState(false)
            ...
            <div className='flex items-center gap-6 relative'>
                <div className='relative'>
                    ...
                    <button onClick={()=> { 
                        ...
                        setShowCreditPop(!showCreaditPopup); setShowUserPop(false)}}
                     className='flex items-center gap-2 bg-gray-100 px-4 py2 rounded-full text-md hover:bg-gray-200 transition'>
                        <BsCoin size={20}/>
                        {userData?.credits || 0 }
                    </button>
                    {showCreaditPopup && (
                        <div className='absolute right-[-50px] mt-3 w-64 bg-white shadow-xl border border-gray-200 rounded-xl p-5 z-50 '>
                            ...
                            <button onClick={()=>navigate("/pricing")} className='w-full bg-black text-white py-2 rounded-lg ttext-sm '>buy more creadits </button>
                        </div>
                    )}
                </div>
                <div className='relative'>
                    <button onClick={()=>{
                        ...
                        setShowUserPop(!showUserPopup); setShowCreditPop(false)}}
                    ...
                    {showUserPopup &&(
                        ...
                        border border-gray-200 rounded-xl p-4 z-50'>
                            ...
                            <button onClick={()=>navigate("history")} className='w-full text-left text-sm py-2
                            hover:text-black text-gray-600 '>InterView history
                            </button>
                            <button onClick={handleLogout} className=' w-full text-left text-sm py-2 flex items-center
                            gap-2 text-red-500'>
