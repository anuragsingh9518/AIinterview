...
export const analyzeResume = async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: "Resume requiired" });
        }
...
export const generateQuestion = async (req, res) => {
    try {
        ...
        mode = mode?.trim();
        if (mode) {
            mode = mode.toLowerCase() === "hr" ? "HR"
                : mode.toLowerCase() === "technical" ? "Technical"
                    : mode;
        }
...
`
        ...
        if (questionsArray.length === 0) {
            console.log("Ai faild to generate questions")
            return res.status(500).json({
                message: "Ai faild to generate questions"
            });
        }
        user.credits -= 50
        await user.save();
...
export const submitAnswer = async (req, res) => {
    try {
        ...
        if (timeTaken > question.timeLimit) {
            question.score = 0;
            question.feedback = "timelimit exced question not evaluated"
...
`
        ...
        const aiResponse = await askAi(messages)
        // Clean markdown fences AI may wrap around JSON (Bug #3 fix)
        ...
        question.score = parsed.finalScore ?? parsed.finalscore;
...
export const finishInterview = async (req, res) => {
    try {
        ...
        const interview = await Interview.findById(interviewId)
        if (!interview) {
            return res.status(400).json({ message: "faild to find interview" })
        }
